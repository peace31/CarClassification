clc;
clear ;
name='F:\Mycompleted task\object tracking\car_detection\train_data1';
    dir name
    listing = dir(name);
    folder_Name={};
...
%               figure();imshow(MB)
            ...
            w1=max(1,logox-RL);
            w2=min(logox+RL,W);
            Ipts=OpenSurf(Im(Fline:Bline,w1:w2),Options);
                if(numel(fieldnames(Ipts))>0)
                    Fflag=1;
                    simage=Im(Fline:Bline,w1:w2);
                end
%             if(w1>logox)
